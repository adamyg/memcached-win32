...
Service::Service(const char *svcname, bool console_mode) :
    ...
    SetVersion(WINMEMCACHED_VERSION_1, WINMEMCACHED_VERSION_2, WINMEMCACHED_VERSION_3);
    SetDescription(WINMEMCACHED_PACKAGE_NAME);
...
Service::ServiceRun()
{
    if (Initialise() >= 0) {
        ...
        case WAIT_OBJECT_0 + 0:                 // service shutdown.
            ...
            break;
        case WAIT_OBJECT_0 + 1:                 // memcached exit.
            diags().info("memcached exit");
            break;
        default:
...
Service::Shutdown()
{
    ...
    if (server_thread_) {
        memcached_signal_stop(1);
        if (WAIT_OBJECT_0 != WaitForSingleObject(server_stopped_event_, 30 * 1000)) {
            diags().warning("unable to shutdown memcached loop");
        }
...
Service::service_body()
{
    ...
    if (! options_.ignore) {
        ...
        // import port, unless an explicit command line option.

        if (! options_.port) {
            DWORD dwValue = 0;
            if (ConfigGet("port", dwValue)) {
                options_.port = (unsigned)dwValue;
            }
        }
        ...
        if (! config_.empty()) {
            const Config::elements_t *elements = 0;
            char section[256] = {0};
                                                // service specific.
            (void) snprintf(section, sizeof(section) - 1, "options.%s", ServiceName());
            if (0 == (elements = config_.GetSectionElements(section))) {

                if (options_.port) {            // port specific.
                    (void) snprintf(section, sizeof(section) - 1, "options.%u", options_.port);
                    elements = config_.GetSectionElements(section);
                }
            ...
            if (elements) {
                char arg[1024] = {0};
                for (Config::elements_t::const_iterator it(elements->begin()), end(elements->end()); it != end; ++it) {
    ...
    if (options_.port) {
        char port[32];
        snprintf(port, sizeof(port) - 1, "--port=%u", options_.port);
        args.push_back(port);
    }
    ...
    const int ret = memcached_main(arguments.argc(), arguments.argv());

    if (ret) {
        diags().fwarning("memcached exited with : %d", ret);
    }
...
Service::ConfigGet(const char *csKey, char *szBuffer, size_t dwSize, unsigned flags)
{
    if (! config_.empty()) {
        ...
        if (NULL != (sep = strchr(csKey, '\\'))) { //TODO: sections
            ...
            const std::string *value = config_.GetValuePtr("", csKey);
            if (value) {
