...
//XXX   ServiceDiags::Syslog::attach(logger_);  // attach logger to syslog.
...
Service::service_body()
{
        ...
        const int ret =
                (options_.service_main ? options_.service_main(arguments.argc(), (const char **) arguments.argv()) : -1);
...
Service::ConfigGet(const char *csKey, std::string &buffer, unsigned flags)
{
        if (! config_.empty()) {
                ...
                if (NULL != (sep = std::strpbrk(csKey, "\\/"))) {
                        value = config_.GetValuePtr(Config::string_view(csKey, sep - csKey), Config::string_view(sep + 1));
                } else {
