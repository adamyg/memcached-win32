...
static const char *	appname = "memcached";
...
main(int argc, const char **argv)
{
	...
	if (verb) {
		//
		//  rpcbind [service_options] run [configuration] ...
		//
			...
			if (ret < 0) {		// command line error.
				if (ret != NTSERVICE_CMD_HELP) {
					diags.ferror("\nsee '%s %shelp' for more details",
						(verb ? verb : "rpcbind_service"), (verb ? "" : "--"));
				}
			}
		...
		//
		//  "rpcbind [service_options] -- [options]" or "rpcbind [service_options]"
		//
		if (argc) {
			--argv, ++argc;
	...
	options.service_main = memcached_main;
	options.service_shutdown = memcached_signal_stop;
