...
static const char *     progname = "memcached";
...
main(int argc, const char **argv)
{
    ...
    // Several options are duplicates of memcached() options, which need to be handled prior to service initialisation.
    ...
    while (NULL == verb && 0 == msg[0] && !do_info &&
        ...
        case 'p': { //-P,--port=<port>
                char *end = 0;
                unsigned port = (unsigned)strtoul(optarg, &end, 10);
    ...
    if (verb) {
        if (0 == _stricmp(verb, "run")) {       // memcached [service_options] run [configuration] ...
            verb = "";
...
help(void)
{
    printf(WINMEMCACHED_PACKAGE " - " WINMEMCACHED_PACKAGE_NAME " " WINMEMCACHED_VERSION ", service options\n\n");

    printf("NAME:\n\n");
    printf("memcached_service - high-performance memory object caching system\n\n");

    printf("SYNOPSIS:\n\n");
    printf("memcached_service [options] <command> [configuration]\n\n");
    ...
    printf("run <configuration>       Run the memcached service.\n");
    printf("\n");

    printf("CONFIGURATION:\n\n");
    printf("These programs follow the usual GNU command line syntax. A summary of configuration options is\n");
    printf("included below. These may also be declared within the configuration file\n\n");
    const char *argv[3] =
        { progname, "--help", NULL };
    memcached_main(2, argv);
}
...
version(void)
{
    printf("%s %s [Build %s %s]\n", WINMEMCACHED_PACKAGE, WINMEMCACHED_VERSION, WINMEMCACHED_BUILD_NUMBER, WINMEMCACHED_BUILD_DATE);
    printf("libevent %s\n", event_get_version());

    const char *argv[3] =
        { progname, "--version", NULL };
    memcached_main(2, argv);
}
