...
{
        if (dir) {
                if ((0 == dir->handle && (dir->handle = FindFirstFileA(dir->path, &dir->fd)) != INVALID_HANDLE_VALUE) ||
                            (INVALID_HANDLE_VALUE != dir->handle && FindNextFileA(dir->handle, &dir->fd))) {
                        dir->entry.d_namlen = strlen(dir->fd.cFileName);
...
{
        if (dir) {
                if (dir->handle && INVALID_HANDLE_VALUE != dir->handle)
...
public:
        ...
        bool start(const Profile &profile, bool toconsole)
        {
                ...
                if (profile_.purge_period_) {   // verify
                        if (profile_.purge_period_ < (profile_.time_period_ * 2)) {
                                profile_.purge_period_ = profile_.time_period_ * 2;
                        }
                }
                ...
                LOGGER_TRACE(if profile_.purge_period_ std::cout << "LOG: purge period: " << profile_.purge_period_ << " seconds\n";)
                ...
                if (stream_.is_open())
                {
                        ...
                        if (profile.purge_period_)
                                purge_time_ = roll_time(now, profile.purge_period_  / 2);
                        thread_handle_ =
...
Logger::Profile::Profile() :
        ...
        purge_period_(7*24*60*60) /*7DAYS*/
{
...
Logger::Profile::line_limit(const char *limit)
{
        ...
        if (line_limit_ < MIN_LINE_LIMIT)
                line_limit_ = MIN_LINE_LIMIT;
