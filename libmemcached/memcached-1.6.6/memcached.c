...
static void process_bin_complete_sasl_auth(conn *c) {
    ...
    char *mech = (char *)_alloca(nkey + 1);
#else
    char *mech = (char *)alloca(nkey + 1);
#endif
#else
    char mech[nkey+1];
#endif
...
#if defined(WIN32PORT)
#if defined(_MSC_VER) || defined(__WATCOMC__)
    char *addr = (char *)_alloca(MAXPATHLEN + extras_len);
    char *svr_addr = (char *)_alloca(MAXPATHLEN + extras_len);
#else
    char *addr = (char *)alloca(MAXPATHLEN + extras_len);
    char *svr_addr = (char *)alloca(MAXPATHLEN + extras_len);
#endif
#else
...
#if defined(WIN32PORT)
        res = c->readfn(c, c->rbuf + c->rbytes, avail);
#else
...
#if defined(WIN32PORT)
            res = c->readfn(c, ch->data + ch->used, (unused > c->rlbytes ? c->rlbytes : unused));
#else
            res = c->read(c, ch->data + ch->used, (unused > c->rlbytes ? c->rlbytes : unused));
#endif
            if (res > 0) {
                pthread_mutex_lock(&c->thread->stats.mutex);
                c->thread->stats.bytes_read += res;
    ...
    while (!stop) {
        ...
        case conn_nread:
            ...
            if ((((item *)c->item)->it_flags & ITEM_CHUNKED) == 0) {
                ...
                /*  now try reading from the socket */
#if defined(WIN32PORT)
                res = c->readfn(c, c->ritem, c->rlbytes);
#else
        ...
        case conn_swallow:
            ...
            /*  now try reading from the socket */
#if defined(WIN32PORT)
            res = c->readfn(c, c->rbuf, c->rsize > c->sbytes ? c->sbytes : c->rsize);
#else
                    ...
                    bool ssl_enabled)
{
    ...
    error = getaddrinfo(xinterface, port_buf, &hints, &ai);
    if (error != 0) {
#if defined(WIN32PORT)
...
#if defined(WIN32PORT)
    fprintf(stderr, "Graceful shutdown signal handled: %d.\n", sig);
#else
...
#if defined(WIN32PORT)
void memcached_signal_stop (int verbose) {
    stop_main_loop = (verbose ? GRACE_STOP : EXIT_NORMALLY);
}
#endif
...
#if defined(SUGHUP) //WIN32PORT
    signal(SIGHUP, sighup_handler);
#endif
#if defined(SUGUSR1) //WIN32PORT
